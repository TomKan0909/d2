...
const BoxElement: React.FC<IBoxElementProps> = ({
    top,
    left,
    imgSource,
    width,
    ...
    <Div top={top} left={left}>
        ...
        {!!text && (
            <AnimatedSmallText textAlign="center">{text}</AnimatedSmallText>
        )}
    </Div>
);
...
const Box: React.FC<IBoxProps> = ({
    imgSource,
    ...props
}): React.ReactElement => <BoxElement imgSource={imgSource} {...props} />;
...
export const BoxComposition: React.FC<IBoxCompositionProps> = ({
    data,
}): React.ReactElement => {
    ...
    return (
        <div>
            {Object.values(data[counter]).map((boxProps) => (
