...
export const TabFeature: React.FC<TabFeatureProps> = ({
     ...
     */
    const listItems = (_listItems: string[]): React.ReactElement[] =>
        _listItems.map(
            ...
            (item, navKey): React.ReactElement => (
                <>
                    <NavTab
                        ...
                        ref={(tab) => {
                            if (tab) tabsRef.current.push(tab);
                        }}
                    >
                        {item.title}
                        {currNavkey === navKey && <MovingDiv ref={movingDiv} />}
                    </NavTab>
                </>
            ),
        );
    ...
    useLayoutEffect(() => {
        ...
        if (nextRect && lastRect.current) {
            ...
            const widthFactor =
                NEUTRAL_SCALE +
                (lastRect.current.width - nextRect.width) / nextRect.width;
            ...
            DataItems.map(
                (item): React.ReactElement => (
                    <Content key={`${item.title}_content`}>
                        ...
                        <RightPanel>
                            <CodeBlock>{item.codeBlock}</CodeBlock>
                        </RightPanel>
                    </Content>
                ),
            ),
    ...
    return (
        <Container {...args}>
            ...
            <Row>
                ...
                <Tabs>
                    <NavTabList>{navigation()}</NavTabList>
                    <ContentHolder>{contentBlock()}</ContentHolder>
                </Tabs>
            </Row>
        </Container>
    );
